...
void Stats::ServerActivate()
{
	RestartRound();
}

void Stats::RestartRound()
{
	g_firstBlood = true;
...
void Stats::PlayerPutInServer(CBasePlayer *player)
{
...
void Stats::TeamScores(TeamName winnerTeam)
{
...
void Stats::PlayerDamaged(CBasePlayer *victim, CBasePlayer *attacker)
{
	if (victim->pev->health <= DANGER_HEALTH)
		PlaySound("saint/mk/danger", victim);
}

void Stats::PlayerDied(CBasePlayer *victim, CBaseEntity *killerEntity, const char *weapon, bool headshot, bool bombkill)
{
...
void Stats::BombPlanted()
{

	//CSGameRules()->m_iC4Timer
}

void Stats::BombDefused()
{

}

void Stats::BombTick(float bombTimeRemaining)
{
...
void Stats::AddStreakKill(CBasePlayer *player)
{
	int index = GetPlayerIndex(player);
	g_streakDeaths[index] = 0;
...
void Stats::AddStreakDeath(CBasePlayer *player)
{
	int index = GetPlayerIndex(player);
	g_streakKills[index] = 0;
...
void Stats::AnnouncePlayersAlive(CBasePlayer *victim)
{
	int tCount = 0;
	int ctCount = 0;
	float tHP = 0;
...
void Stats::AnnounceHeadshot(CBasePlayer *victim, CBasePlayer *killer, const char *weapon)
{
...
void Stats::AnnounceKnifeKill(CBasePlayer *victim, CBasePlayer *killer)
{
...
void Stats::AnnounceWorldspawnKill(CBasePlayer *victim)
{
...
void Stats::AnnounceGrenadeKill(CBasePlayer *victim, CBasePlayer *killer)
{
	char *msg;
	SetHudMessage(255, 100, 100, -1.0, 0.25, 1, 6.0, 6.0, 0.5, 0.15, -1);
...
void Stats::CheckDoubleKill(CBasePlayer *killer)
{
...
const char *Stats::ConvertToSaintWeapons(const char *wpn)
{
...
void Stats::ClientCmd(const char *command, CBasePlayer *player)
{
	if (player)
	{
...
void Stats::NotifyMessage(char *msg, int msg_dest, CBasePlayer *player)
{
	msg = UTIL_VarArgs("%s\n", msg);
...
void Stats::SetHudMessage(byte r, byte g, byte b, float x, float y, int effect, float fxTime, float holdTime, float fadeinTime, float fadeoutTime, int channel)
{
	g_hudset.a1 = 0;
	g_hudset.a2 = 0;
	g_hudset.r2 = 255;
...
void Stats::ShowHudMessage(const char *msg)
{
	UTIL_HudMessageAll(g_hudset, msg);
}

void Stats::PlaySound(const char *snd, CBasePlayer *player)
{
	char cmd[255];
	strcpy(cmd, "spk ");
	strcat(cmd, snd);
...
int Stats::GetPlayerIndex(CBasePlayer *player)
{
	for (int iIndex = 1; iIndex <= gpGlobals->maxClients; ++iIndex)
	{
